...
namespace Jackson_Brynne_HW3.Seeding
{
	public static class SeedBooks
	{
		public static void SeedAllBooks(AppDbContext db)
		{
			...
			List<Book> Books = new List<Book>(); //create a list that is empty and contains books called Books
			...
			try
			{
				Book b1 = new Book()
				{
					...
					Price = 21.95m,
					BookFormat = Format.Paperback //enum from book model class
				};
				//db.Genres is select * from genres, firstor default means first one that is true
				//g goes to g.GenreName where it equals contemp fiction
				//basically this is SELECT * from Genres where GenreName is contemp fiction
				...
				try
				{
					foreach (Book bookToAdd in Books)
					{
						...
						//see if the book already exists in the database
						Book dbBook = db.Books.FirstOrDefault(b => b.Title == bookToAdd.Title);
						if (dbBook == null) //this title doesn't exist which means book isn't in database
						{
						...
						else //book exists - update values; reset all fields except unique ID
						{
